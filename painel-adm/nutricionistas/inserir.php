...
$caminho = '../../img/nutricionistas/' .$nome_img;
...
if($id == ""){
	...
	$insertNutricionista = $pdo->prepare("INSERT INTO NUTRICIONISTA SET CODPESSOA = :codpessoa, CRN = :crn");	
	...
	$insertNutricionista->bindValue(":codpessoa", $codpessoa);
	$insertNutricionista->bindValue(":crn", $crn);
	$insertNutricionista->execute();

}else{
	if($imagem == "sem-foto.jpg") {
		$edtPessoa = $pdo->prepare("UPDATE PESSOA SET NOME = :nome, CPF = :cpf, SEXO = :sexo, EMAIL = :email, telefone = :telefone WHERE ID = '$id'");
		$edtNutricionista = $pdo->prepare("UPDATE NUTRICIONISTA SET CRN = :crn WHERE CODPESSOA = '$id'");	
	} else {
